...
def MinMaxScaler():
        ...
        DF_Jordan_u_3['Day']=DF_Jordan_u.index
...
def Plotting():
        ...
        DF_Jordan_u_3 = pd.read_csv('/opt/airflow/data/DF_Jordan_Scaled.csv', parse_dates=['Day'])
        DF_Jordan_u_3.set_index('Day', inplace=True) 
        Select_Columns=['Confirmed','Deaths', 'Recovered', 'Active','Case_Fatality_Ratio']
...
def CSV_to_Postgres():
        ...
        DF_Jordan_Not_Scaled = pd.read_csv('/opt/airflow/data/DF_Jordan.csv', parse_dates=['Day'])
        DF_Jordan_Not_Scaled.set_index('Day', inplace=True)

        DF_Jordan_Scaled = pd.read_csv('/opt/airflow/data/DF_Jordan_Scaled.csv', parse_dates=['Day'])
        ...
        DF_Jordan_Not_Scaled.to_sql('Covid19_Not_Scaled', engine,if_exists='replace',index=False)    
...
with DAG('Jordan_COVID19_dag',
    ...
    Extract_Data = PythonOperator(task_id='Extract_Jordan_Data', python_callable=Create_data)
    Scaled_Data = PythonOperator(task_id='Scale_Jordan_Data', python_callable=MinMaxScaler)
    Ploting_Data = PythonOperator(task_id='Plot_Jordan_Data', python_callable=Plotting)
